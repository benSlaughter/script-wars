...
<div class="stats-page">
	<div class="stats-header">
		<h1>📈 {data.gameName} Script Stats</h1>
		<a href={resolve(`/games/${data.gameId}/scripts`)} class="btn btn-ghost">← Back to Scripts</a>
	</div>
	...
	{:else}
		{#each data.scriptStats as script (script.id)}
			<div class="card script-stats" class:active={script.isActive}>
				...
				{#if script.opponents.length > 0}
					<table>
						...
						<tbody>
							{#each script.opponents as opp (opp.name)}
								<tr>
									<td>{opp.name}</td>
