<script lang="ts">
	...
	async function activateScript(id: string) {
		const res = await fetch('/api/scripts/activate', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ id })
		});
...
{:else}
	...
	{#if scripts.length === 0}
		<div class="card empty">
			<p>You haven't written any scripts yet. Time to enter the arena!</p>
			<a href="/scripts/new" class="btn btn-primary">Write Your First Bot</a>
		</div>
	{:else}
				...
				<div class="card script-card" class:active={script.isActiveEntry}>
					<div class="script-header">
						...
						<div class="script-actions">
							...
							{/if}
							<a href="/scripts/{script.id}/edit" class="btn btn-ghost btn-sm">Edit</a>
							<button
